...
- A cell is excluded when it is NaN on either side, or equal to that file's fill
  ...
  netCDF's own meaning of "never written".
...
`eps = DBL_EPSILON` guarding a zero reference); a variable passes if every
compared cell does. `abs` is the field's noise floor -- directions get `1e-2`
degrees because `dir`/`dp` are quantised by the directional bin width.
...
```
variable         n      max|d|         rms     max rel  verdict
----------------------------------------------------------------
hs               3   0.000e+00   0.000e+00   0.000e+00  PASS
fp               4   1.000e-01   5.000e-02   2.500e-01  FAIL
foo              4   9.000e+00   4.500e+00   9.000e+00  unlisted
nccmp-tol: FAIL (2 judged, 1 unlisted)
```

`n` is the number of cells compared after exclusions; `max|d|` and `rms` are
absolute, `max rel` is `max |d| / max(|ref|, eps)` over the same cells.
