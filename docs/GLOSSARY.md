...
| NC4 | ⚠ legacy "enable netCDF-4 output" key; **not in `switches.json`, no `W3_NC4` guard, in 1 of 558 upstream switch files** `(v)` — the course's claim that `ww3_ounf` writes nothing without it is unverified for the CMake build, where netCDF is a `find_package` dependency | Inert in 7.14; `nc-config --has-nc4` is what the manual (`impl/compile.tex`) asks you to check instead. | [course/01-build.md](../course/01-build.md), [course/09-benchmark-profile-compile-run.md](../course/09-benchmark-profile-compile-run.md) |
...
| NCO / ncks / ncap2 / ncdiff | the netCDF Operators: subsetting (`ncks -v hs -d time,-1`), arithmetic (`ncap2`), differencing (`ncdiff`) `(v)` [course/06-output.md](../course/06-output.md), [course/04-forcing.md](../course/04-forcing.md) | ⚠ not in the `just toolchain` listing; install on the host. | [course/03-grids.md](../course/03-grids.md) |
...
| cdo | ⚠ Climate Data Operators (not expanded in the repo) | `cdo mergetime` as an alternative GRIB concatenation step in `get_gfs.sh`. | [course/04-forcing.md](../course/04-forcing.md) |
