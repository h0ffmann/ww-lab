...
2. **A "Proposta de Projeto de Graduação"** for UFRJ's Escola Politécnica / DEL, in Portuguese
   (source of truth) and English (generated), in the department's proposal format: header block,
...
pubs/
  ...
  proposal/
    pt/                        SOURCE: 01-titulo.md … 08-cronograma.md, one numbered section per file
    ...
    meta.en.yaml               same keys in English (hand-written once)
...
2. `nix build .#proposal-pt` with the eight PT section files produces a PDF whose first page
...
- The PT text itself is a writing task on top of this pipeline: first draft from
