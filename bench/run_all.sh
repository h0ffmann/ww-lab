...
#
# The WW3 case is generated by ww_bench_case, the C++ generator built in the
# kokkos/ tree (`just kokkos-build openmp-release`, or set WW_BENCH_CASE to any
# other build of it). Runs from any cwd: everything happens inside bench/.
set -euo pipefail

cd "$(dirname "$0")"

WW3BUILD="${1:-${WW3:-}/build}"
BENCH_CASE="${WW_BENCH_CASE:-../kokkos/build/openmp-release/tools/bench_case/ww_bench_case}"
...
echo "############################################################"
if [ -d "$WW3BUILD" ]; then
  if [ ! -d case_medium ]; then
    [ -x "$BENCH_CASE" ] || {
      echo "!! no ww_bench_case at '$BENCH_CASE'."
      echo "   Build it first:  just kokkos-build openmp-release"
      echo "   (or point WW_BENCH_CASE at another build of it)"
      exit 1
    }
    "$BENCH_CASE" --size medium -o case_medium
  fi
